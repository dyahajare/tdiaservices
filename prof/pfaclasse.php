...
try {
    ...
    $req_classes = "
     SELECT DISTINCT d.classe_pfa
     FROM pfa d
     WHERE d.email_encadrant = :email_prof
    ";
...
<div class="container mt-5">
    ...
    <div class="list-group mb-4">
        <?php
        if (!empty($classes)) {
            foreach ($classes as $pfa) {
                echo '<a href="pfa.php?classe_pfa=' . urlencode($pfa['classe_pfa']) . '" class="list-group-item list-group-item-action">' . htmlspecialchars($pfa['classe_pfa']) . '</a>';
            }
        } else {
