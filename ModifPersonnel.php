...
if (isset($_POST['submit'])) {
    ...
    if (isset($_FILES['photo']) && $_FILES['photo']['error'] === UPLOAD_ERR_OK) {
        ...
        if ($result) {
            ...
            // Gestion de l'erreur si le CIN n'est pas trouvé dans la base de données
            echo "Erreur : CIN non trouvé dans la base de données.<br>";
        }
    }

    // Préparer la requête de mise à jour
    $sql = "UPDATE personnel SET nom_personnel = ?, prenom_personnel = ? , emailPro_personnel = ?, emailPerso_personnel = ?, telephone = ?, Statut = ?, Situation_famillial = ?, sexe = ?, photo_personnel = ?, mot_pass = ? WHERE cin = ?";
    $stmt = $conn->prepare($sql);
    $params = array($nom, $prenom, $emailPro, $emailPerso, $telephone, $Statut, $SituationFamiliale, $Sexe, $img, $mot_pass, $cin);
    $stmt->execute($params);
            ...
            <div class="col-lg-10 col-md-10 col-sm-12 col-xs-12" id="d1">
                ...
                <div class="card mx-auto" style="max-width: 700px; border-radius: 10px;background-color: rgba(19, 19, 97, 0.4);">
                    ...
                    <div class="card-body" style="color: #FFE4C4;">
                        <form action="modifPersonnel.php?cin=<?php echo $cin ?>" method="POST" enctype="multipart/form-data">
                            <div class="form-group">
                                <label for="photo">Photo :</label>
                                <img src="uploads/<?php echo htmlspecialchars($perso['photo_personnel']); ?>" alt="Image" width="100px" height="100px">
                                <input type="file" id="photo" name="photo" class="form-control" value="<?php echo $profileImage ?>" >
                            </div>
                            <div class="form-group">
                                <label for="nom">Nom :</label>
                                <input type="text" id="nom" name="nom"  class="form-control" value="<?php echo htmlspecialchars($perso['nom_personnel']); ?>">
                            </div>
                            <div class="form-group">
                                <label for="prenom">Prénom :</label>
                                <input type="text" id="prenom" name="prenom" class="form-control" value="<?php echo htmlspecialchars($perso['prenom_personnel']); ?>">
                            </div>
                            <div class="form-group">
                                <label for="emailPro">Email Professionnel :</label>
                                <input type="email" id="emailPro" name="emailPro"  class="form-control" value="<?php echo htmlspecialchars($perso['emailPro_personnel']); ?>">
                            </div>
                            <div class="form-group">
                                <label for="emailPerso">Email Personnel :</label>
                                <input type="email" id="emailPerso" name="emailPerso" class="form-control" value="<?php echo htmlspecialchars($perso['emailPerso_personnel']); ?>">
                            </div>
                            <div class="form-group">
                                <label for="telephone">Téléphone :</label>
                                <input type="number" id="telephone" name="telephone"  class="form-control" value="<?php echo htmlspecialchars($perso['telephone']); ?>">
                            </div>
                            <div class="form-group">
                                <label for="Statut">Statut :</label>
                                <input type="text" id="Statut" name="Statut"  class="form-control" value="<?php echo htmlspecialchars($perso['Statut']); ?>">
                            </div>
                            <div class="form-group">
                                <label for="SituationFamiliale">Situation Familiale :</label>
                                <input type="text" id="SituationFamiliale" name="SituationFamiliale"  class="form-control" value="<?php echo htmlspecialchars($perso['Situation_famillial']); ?>">
                            </div>
                            <div class="form-group">
                                <label for="Sexe">Sexe :</label>
                                <select name="Sexe" id="Sexe"  class="form-control">
                                    <option value="Homme" <?php echo ($perso['sexe'] == 'Homme') ? 'selected' : ''; ?>>Homme</option>
                                    <option value="Femme" <?php echo ($perso['sexe'] == 'Femme') ? 'selected' : ''; ?>>Femme</option>
                                </select>
                            </div>
                            <div class="form-group">
                                <label for="mot_pass">Mot de passe :</label>
                                <input type="text" id="mot_pass" name="mot_pass"  class="form-control" value="<?php echo htmlspecialchars($perso['mot_pass']); ?>">
                            </div>
