...
$sp->execute();
$res=$sp->fetchAll();
$requ=('SELECT * FROM notif WHERE classe_notif=$id');
$spu=$con->prepare($req);
$resu=$spu->fetchAll();
?>
<!DOCTYPE html>
...
<head>
   ...
   <div class="dropdown-list dropdown-menu dropdown-menu-right shadow " aria-labelledby="alertsDropdown">
           ...
           <div class="row1" style=" background: linear-gradient(to bottom,   rgb(204, 202, 202), rgb(249, 249, 250));"> 
               ...
               <div class="row">
               <div class="card mb-4" style="margin-left:3%;">
       <strong style="color:blue;padding-left:2%;">Avis</strong><br>
           <div class="card-header">
           <?php
        if (isset($_GET['titre_actu']) && isset($_GET['pdf_path'])) {
       $titre_actu = urldecode($_GET['titre_actu']);
       $_SESSION['titre_actu'] = $titre_actu;
       $_pdfpath=urldecode($_GET['pdf_path']);
       $_SESSION['msg_actu']=$_pdfpath;
}
?>
       <strong><?php echo($_SESSION['titre_actu']); ?></strong></div>
           <div class="card-body">
           <p> L'annonce complète est disponible via le fichier attaché ci-dessous: <br>
   <a href="<?php echo htmlspecialchars('../uploads/' . $_SESSION['msg_actu']); ?>" download="<?php echo htmlspecialchars($_SESSION['msg_actu']); ?>">Lire l'annonce</a>
</p>
   </div></div>
