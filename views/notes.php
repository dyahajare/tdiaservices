...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $allGrades = [];
    $rq = $pdo->prepare("SELECT Note FROM classe WHERE nom = ?");
    $rq->execute([$nom_classe]);
    $noteStatus = $rq->fetchColumn();

    $allGrades = [];
    if ($noteStatus == 1) { // Check if notes are available
        foreach ($moduleData as $module) {
            $filePath = realpath('../prof/src/uploads/' . $module['notes']); // Use realpath for Windows compatibility
            if (!empty($filePath) && file_exists($filePath)) {
                $moduleGrades = processGradesFile($filePath);
                $module['grades'] = $moduleGrades;
                $allGrades[] = $module;
            } else {
                echo "File path for module " . $module['nom_module'] . " is empty or the file does not exist.";
            }
        }
    } else {
        echo "Notes indisponibles !";
    }
}
?>
