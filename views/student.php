...
$req ='SELECT * FROM notif WHERE classe_notif=?';  
$psu = $con->prepare($req);
$para=array($id);     
$psu->execute($para); 
$resu= $psu->fetchAll(); 
...
$archived = 0;
$requ = 'SELECT * FROM cours WHERE classe_cours=? AND archived=?';  
$psus = $con->prepare($requ);
$paras = array($id, $archived);     
$psus->execute($paras); 
$resul = $psus->fetchAll();


$requette = 'SELECT * FROM cours WHERE classe_cours = ?';
$psut = $con->prepare($requette);
$param = array($id);
$psut->execute($param);
$resultat = $psut->fetchAll();

if (!empty($resultat)) {
    $cinprof = $resultat[0]['cin_prof'];
    $requett = 'SELECT * FROM professeur WHERE cin = ?';
    $psute = $con->prepare($requett);
    $parame = array($cinprof);
    $psute->execute($parame);
    $resultate = $psute->fetchAll();
}
?><!DOCTYPE html>
<html>
<head>
                    ...
                    <div class="row">
                        <div class="col-md-4">
                            <div class="Avisbar">
                              ...
                              <?php
                            $image_names = $ligne['img_actu'] ;
                            $image_paths = "../uploads/". $image_names; ?>
...
<?php } ?>
                                    ...
                                    <div class="notifsbar" style="padding-left: 30px;">
                                      ...
                                      <?php foreach($resu as $ligne){;?>
                                        ...
                                        <a href="notifhandler.php?titre_notif=<?php echo urlencode($ligne["titre_notif"]); ?>&msg_notif=<?php echo urlencode($ligne["msg_notif"]); ?>" style="color:#77787a;text-decoration:none;padding-left:1%;"><?php echo $ligne['titre_notif'];?><br><br></a>
                                        <?php }?>
                                    </div>
                                </div> 
