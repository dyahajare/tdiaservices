...
$req = $conn->prepare("SELECT * FROM demandes LIMIT :limit OFFSET :offset");
...
$total_req = $conn->prepare("SELECT COUNT(*) as total FROM demandes");
$total_req->execute();
...
if(isset($_POST['valider'])){
    // Récupérer l'ID de la demande à partir de $_POST
    $demande_id = $_POST['demande_id'];
    // Préparer et exécuter la requête de mise à jour
    $req = $conn->prepare("UPDATE demandes SET réalisé = 1 WHERE id = :demande_id"); // Modifier pour utiliser l'ID de la demande
...
<body>
                        ...
                        <tr>
                            ...
                            <td>
                                <?php if ($row['réalisé'] == 0): ?>
                                    <form method="post" action="gestionDemande.php">
    ...
    <script>
        ...
        // Fonction pour mettre à jour le statut de la demande et masquer le bouton
        function validerDemande(idDemande) {
            $.ajax({
                type: "POST",
                url: "gestionDemande.php",
                data: {
                    demande_id: idDemande
                },
                success: function(response) {
                    $("#valider_" + idDemande).replaceWith("<span>Réalisé</span>");
                }
            });
        }

        $(document).on("click", "[id^='valider_']", function() {
            var idDemande = this.id.split("_")[1];
